...
function createEmitCallback(options, tsickle) {
  ...
  }, host, options2, targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, {
...
function readNgcCommandLineAndConfiguration(args) {
  const options = {};
  const parsedArgs = yargs(args).parserConfiguration({"strip-aliased": true}).option("i18nFile", {type: "string"}).option("i18nFormat", {type: "string"}).option("locale", {type: "string"}).option("missingTranslation", {
    type: "string",
    choices: ["error", "warning", "ignore"]
  }).option("outFile", {type: "string"}).option("watch", {type: "boolean", alias: ["w"]}).parseSync();
