...
var ValueConverter = class extends AstMemoryEfficientTransformer {
  ...
  visitLiteralMap(map, context) {
    return new BuiltinFunctionCall(map.span, map.sourceSpan, this.visitAll(map.values), (values) => {
      const literal2 = literalMap(values.map((value, index2) => ({
        key: map.keys[index2].key,
        value,
        quoted: map.keys[index2].quoted
      })));
      return getLiteralFactory(this.constantPool, literal2, this.allocatePureFunctionSlots);
    });
  }
...
function parseTemplate(template2, templateUrl, options = {}) {
  ...
  const {
    ...
    ngContentSelectors,
    commentNodes
  } = htmlAstToRender3Ast(rootNodes, bindingParser, {collectCommentNodes: !!options.collectCommentNodes});
...
function convertDeclareComponentFacadeToMetadata(decl2, typeSourceSpan, sourceMapUrl) {
  ...
  const {
    template: template2,
    interpolation
  } = parseJitTemplate(decl2.template, decl2.type.name, sourceMapUrl, (_a2 = decl2.preserveWhitespaces) != null ? _a2 : false, decl2.interpolation);
  const declarations = [];
...
function migrateTypographyConfigs(content, namespace) {
  ...
  replacements.sort((a, b) => b.start - a.start).forEach(({
                                                            start,
                                                            end,
                                                            text: text2
                                                          }) => content = content.slice(0, start) + text2 + content.slice(end));
  return content;
}
