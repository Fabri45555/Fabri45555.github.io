...
@mixin legacy-color($config-or-theme) {
  ...
  $underline-color: theming.get-color-from-palette($foreground, divider, if($is-dark-theme, 0.7, 0.42));
...
@mixin _label-floating($font-scale, $infix-padding, $infix-margin-top) {
  ...
  // translateZ fixes.
  transform: translateY(-$infix-margin-top - $infix-padding) scale($font-scale) perspective(100px) translateZ(0.001px + $legacy-dedupe);
...
@mixin _label-floating-print($font-scale, $infix-padding, $infix-margin-top) {
  // This results in a small jitter after the label floats on Firefox, which the
  // translateZ fixes.
  transform: translateY(-$infix-margin-top - $infix-padding + $legacy-dedupe) scale($font-scale);
