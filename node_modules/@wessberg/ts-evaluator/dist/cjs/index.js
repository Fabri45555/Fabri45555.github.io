 ...
 * @type {TrapConditionMap<NodeBuiltInsAndGlobals>}
 */
const NONDETERMINISTIC_MAP = Object.assign(Object.assign({}, NETWORK_MAP), {
  Math: {
    random: {
      ["__$$_PROXY_APPLY" /* APPLY */]: true
    }
...
class NonDeterministicError extends PolicyError {
  constructor({
                operation,
                node,
                message = `The operation: '${operation}' is nondeterministic. That is in violation of the policy`
              }) {
  ...
  const hook = (item) => {
    ...
    if (policy.deterministic && isNonDeterministic(item)) {
      throw new NonDeterministicError({
        operation: stringifyPolicyTrapKindOnPath(item.kind, item.path),
        node: getCurrentNode()
      });
    }
    return true;
  ...
  if (objectPath.has(env.env, firstBinding)) {
    ...
    switch (path) {
      ...
      case "__filename": {
        ...
        return preset === exports.EnvironmentPresetKind.NODE && typeof literal === "function" && node != null ? {literal: literal(node.getSourceFile().fileName)} : {literal};
      }
      default:
        return {literal};
    }
 ...
 */
function evaluateParameterDeclarations({
                                         node,
                                         evaluate,
                                         environment,
                                         statementTraversalStack,
                                         typescript
                                       }, boundArguments, context) {
...
function evaluateArrowFunctionExpression(options) {
  ...
  const arrowFunctionExpression = hasModifier(node, typescript.SyntaxKind.AsyncKeyword)
    ? async (...args) => {
      ...
      evaluateParameterDeclarations(Object.assign(Object.assign({}, options), {
...
function evaluateFunctionDeclaration(options) {
  ...
  if (nameResult != null) {
    setInLexicalEnvironment({
      env: environment,
      path: nameResult,
      value: _functionDeclaration.bind(_functionDeclaration),
      reporting,
      node
    });
  }
...
function evaluateFunctionExpression(options) {
  ...
  if (nameResult != null) {
    setInLexicalEnvironment({
      env: environment,
      path: nameResult,
      value: _functionExpression.bind(_functionExpression),
      reporting,
      node
    });
  }
 ...
 * @returns
 */
function evaluateClassExpression({
                                   node,
                                   environment,
                                   evaluate,
                                   stack,
 ...
 */
function evaluateSourceFileAsNamespaceObject({
                                               node,
                                               environment,
                                               evaluate,
                                               typeChecker,
 ...
 */
function evaluateGetAccessorDeclaration({
                                          node,
                                          environment,
                                          evaluate,
                                          stack,
  ...
  Object.defineProperty(parent, nameResult, Object.assign(Object.assign({}, currentPropertyDescriptor), {
    configurable: true,
 ...
 */
function evaluateBindingElement({
                                  environment,
                                  node,
                                  evaluate,
                                  logger,
...
function evaluateDecorator({
  ...
  stack.push(tslib.__decorate([index != null ? tslib.__param(index, decoratorImplementation) : decoratorImplementation], parent, propertyName));
}
...
class Logger {
  ...
  logContinue(node, typescript) {
    if (this.logLevel < 30 /* DEBUG */)
      return;
    console.log(`${chalk__default['default'].yellow(`continue`)} encountered within ${chalk__default['default'].yellow(stringifySyntaxKind(node.kind, typescript))}`);
  }
  ...
  logBreak(node, typescript) {
    if (this.logLevel < 30 /* DEBUG */)
      return;
    console.log(`${chalk__default['default'].yellow(`break`)} encountered within ${chalk__default['default'].yellow(stringifySyntaxKind(node.kind, typescript))}`);
  }
  ...
  logReturn(node, typescript) {
    if (this.logLevel < 30 /* DEBUG */)
      return;
    console.log(`${chalk__default['default'].yellow(`return`)} encountered within ${chalk__default['default'].yellow(stringifySyntaxKind(node.kind, typescript))}`);
  }
  ...
  logResult(result, intermediateContext) {
    ...
    if (intermediateContext != null) {
      console.log(chalk__default['default'].gray(`(intermediate value from context '${intermediateContext}'): `), chalk__default['default'].green(`[RESULT]:`), this.compactValue(result));
    } else
      console.log(chalk__default['default'].green(`[RESULT]:`), result);
  }
  ...
  logNode(node, typescript, context) {
    ...
    if (context != null)
      headRaw += chalk__default['default'].cyan(`(${context})`);
    const tailRaw = node.getText();
    const head = chalk__default['default'].yellow(headRaw);
    const tail = chalk__default['default'].gray(tailRaw);
    console.log(head);
    console.log(tail);
  }
  ...
  logBinding(lValue, rValue, scope) {
    if (this.logLevel < 20 /* VERBOSE */)
      return;
    console.log(`${scope == null ? "" : chalk__default['default'].green(`(${scope}): `)}${chalk__default['default'].red(lValue)} ->`, this.compactValue(rValue));
  }
  ...
  logHeritage(classDeclaration) {
    ...
    if (parent.toString().includes("[Class")) {
      console.log(`${chalk__default['default'].cyan(classDeclaration.toString())} ${chalk__default['default'].yellow("extends")} ${chalk__default['default'].cyan(parent.toString())}`);
    }
  }
  ...
  logStack(stack) {
    if (this.logLevel < 30 /* DEBUG */)
      return;
    console.log(`Stack value: ${chalk__default['default'].blue(stringifyLiteral(this.compactValue(stack.lastItem)))}`);
  }
  ...
  logStatementTraversalStack(stack, typescript) {
    if (this.logLevel < 30 /* DEBUG */)
      return;
    console.log(`[${stack.map(kind => chalk__default['default'].blue(stringifySyntaxKind(kind, typescript))).join(", ")}]`);
  }
...
function evaluate({
                    typeChecker,
                    node,
                    environment: {preset = exports.EnvironmentPresetKind.NODE, extra = {}} = {},
                    typescript = TSModule__namespace,
                    logLevel = 0 /* SILENT */,
                    policy: {
                      deterministic = false,
                      network = false,
